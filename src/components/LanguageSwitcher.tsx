...
export function LanguageSwitcher({ value, onChange, compact = false }: LanguageSwitcherProps) {
  return (
    <div className="flex flex-wrap items-center justify-center gap-2 rounded-full border border-white/15 bg-black/25 p-1.5 backdrop-blur-md">
      {LANGUAGES.map((language) => (
        <button
          key={language.code}
          type="button"
          onClick={() => onChange(language.code)}
          className={`rounded-full px-3 py-2 text-xs font-semibold uppercase text-cream transition duration-200 ${
            value === language.code
